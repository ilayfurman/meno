...
export function CookbookScreen() {
  ...
  const [pendingFilters, setPendingFilters] = useState<CookbookFilterState>(
    defaultCookbookFilters,
  );
  const [appliedFilters, setAppliedFilters] = useState<CookbookFilterState>(
    defaultCookbookFilters,
  );
  ...
  const [agentSelectedContextIds, setAgentSelectedContextIds] = useState<
    string[]
  >([]);
  const navigation =
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    if (quickFilters.has("comfort")) {
      result = result.filter((recipe) =>
        recipe.short_hook.toLowerCase().includes("comfort"),
      );
    }
    ...
    if (quickFilters.has("veg")) {
      result = result.filter((recipe) =>
        recipe.dietary_tags.some((tag) => tag.toLowerCase().includes("veg")),
      );
    }

    if (appliedFilters.difficulty.length > 0) {
      const allowed = new Set(
        appliedFilters.difficulty.map((item) => item.toLowerCase()),
      );
      result = result.filter((recipe) =>
        allowed.has(recipe.difficulty.toLowerCase()),
      );
    }

    if (appliedFilters.cuisines.length > 0) {
      const allowed = new Set(
        appliedFilters.cuisines.map((item) => item.toLowerCase()),
      );
      result = result.filter((recipe) =>
        allowed.has(recipe.cuisine.toLowerCase()),
      );
    }
    ...
    if (sortBy === "title_asc") {
      ...
      result = [...result].sort(
        (a, b) => a.total_time_minutes - b.total_time_minutes,
      );
    } else if (sortBy === "time_desc") {
      result = [...result].sort(
        (a, b) => b.total_time_minutes - a.total_time_minutes,
      );
    }
    ...
    Alert.alert("Remove recipe", "Remove this recipe from cookbook?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: () => {
          ...
          void removeRecipeFromCookbook(id).catch((error) => {
            setRecipes(previous);
            Alert.alert(
              "Remove failed",
              error instanceof Error ? error.message : "Could not remove recipe.",
            );
          });
        },
      },
  ...
  const onDeleteSelected = () => {
    ...
    Alert.alert(
      "Remove selected",
      `Remove ${selectedIds.length} selected recipe(s)?`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Remove",
          ...
          onPress: () => {
            ...
            const previous = recipes;
            setRecipes((prev) =>
              prev.filter((recipe) => !idsToRemove.includes(recipe.id)),
            );
            setSelectedIds([]);
            void removeRecipesFromCookbook(idsToRemove).catch((error) => {
              setRecipes(previous);
              Alert.alert(
                "Remove failed",
                error instanceof Error ? error.message : "Could not remove recipes.",
              );
            });
          },
        },
  ...
  const askAgent = async () => {
    ...
    const contextRecipes =
      agentSelectedContextIds.length > 0
        ? recipes.filter((recipe) =>
            agentSelectedContextIds.includes(recipe.id),
          )
        : recipes.slice(0, 6);
    ...
    if (contextRecipes.length === 0) {
      Alert.alert(
        ...
        "Save recipes first or select recipes before opening the agent.",
      );
      return;
    }
  ...
  return (
    <View style={styles.shell}>
      <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
        <View style={styles.titleRow}>
          ...
          <View style={styles.topActions}>
            <Pressable
              style={[
                styles.headerButton,
                mode === "select" && styles.headerButtonActive,
              ]}
              onPress={() =>
                setCookbookMode(mode === "select" ? "browse" : "select")
              }
            >
            ...
            <Pressable
              style={[
                styles.headerButton,
                mode === "reorder" && styles.headerButtonActive,
              ]}
              onPress={() =>
                setCookbookMode(mode === "reorder" ? "browse" : "reorder")
              }
            >
              <Text style={styles.headerButtonText}>
        ...
        {mode === "reorder" ? (
          <View style={styles.reorderHintCard}>
            ...
            {reorderFromId ? (
              <Pressable
                style={styles.clearPickButton}
                onPress={() => setReorderFromId(null)}
              >
        ...
        {displayRecipes.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyBody}>
              Save from Results or Recipe Detail to build your cookbook.
            </Text>
          </View>
        ) : (
          <View style={styles.grid}>
            {displayRecipes.map((recipe) => {
              const selected =
                selectedIds.includes(recipe.id) || reorderFromId === recipe.id;
              return (
                  ...
                  {mode === "reorder" ? (
                    <View style={styles.reorderBadge}>
                      <Text style={styles.reorderBadgeText}>
                        {reorderFromId === recipe.id ? "Picked" : "Tap to move"}
                      </Text>
                    </View>
                  ) : null}
                </View>
      ...
      {mode === "select" ? (
        ...
        >
          {NativeBlurView ? (
            ...
            <View
              style={[StyleSheet.absoluteFill, styles.selectActionFallbackBg]}
            />
          )}
          <View style={styles.selectActionRow}>
            ...
            <Text
              style={[
                ...
                { bottom: selectDockBottomPad + 10 },
              ]}
            >
              {selectedIds.length > 0
                ? `${selectedIds.length} Recipes Selected`
                : "Select Recipes"}
            </Text>
          </View>
        </View>
      ...
      {mode !== "select" ? (
        <FloatingAgentButton
          onPress={openAgent}
          bottom={Math.max(insets.bottom + 56, 70)}
        />
      ) : null}
